...
import pickle
import pandas as pd
from conf_manager import CSV_PATH, STAGING_PATH  # pylint: disable=import-error
...
def create_fact_table(df, passenger_count, trip_distance, rate_code, pickup_location,
    ...
    face_table = df.merge(passenger_count, on='passenger_count') \
        ...
        .merge(payment_type, on='payment_type')[['VendorID', 'datetime_id', 'passenger_count_id', 'trip_distance_id',
...
def save_formatted_data(formatted_data):
    """function to pickled transformed data.
    Saveds data.pickled to staging folder"""
    with open(f"{os.path.join(STAGING_PATH, 'data.pickle')}", 'wb') as output:
        pickle.dump(formatted_data, output, pickle.HIGHEST_PROTOCOL)
        print("Data has been pickled")
    return True

if __name__ == '__main__':
    orm_data_dict = transformations(CSV_PATH)
    save_formatted_data(orm_data_dict)
print("Data transformation and saving to staging folder complete!")
